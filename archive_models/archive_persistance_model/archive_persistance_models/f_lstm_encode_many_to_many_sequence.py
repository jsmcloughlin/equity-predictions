...
'''
[10, 100, 300, 32, 1, 5, 0.07, 'relu', 0] 1.947692972668327
[10, 200, 200, 32, 1, 5, 0.05, 'relu', 0] 1.9496239124363004
[10, 200, 200, 16, 1, 5, 0.05, 'tanh', 0] 1.9497144742512897
[10, 200, 500, 32, 1, 5, 0.07, 'tanh', 0] 1.953374776398432
[10, 100, 500, 64, 1, 5, 0.1, 'relu', 0] 1.9535379580140753
'''
...
def build_model(train, test, config):
	...
	model.add(LSTM(n_nodes, input_shape=(n_timesteps, n_features)))
	model.add(Activation(n_actfn))
	model.add(RepeatVector(n_outputs))
	model.add(LSTM(n_nodes, return_sequences=True))
	model.add(Activation(n_actfn))
	model.add(Dropout(n_dropout))
	model.add(TimeDistributed(Dense(n_nodes//2)))
	model.add(Activation(n_actfn))
...
cfg_list = random.choices(aapl.model_configs(), k=20) #Run only a selection of configurations thrrough the model.
cfg_list
